...
void loop() {
  yedek2 = digitalRead(23);
  yedek = digitalRead(33);
  if(yedek == false || yedek2 == true){
    yedeks = true;
    digitalWrite(2,HIGH);
  ...
  if(h_yeni >= 350 && yedeks){ // Roketin Atılmasından önce 2250 metre yapılacak.
    tepe_noktasi = true; 
    Serial.println("Tepe Noktasina Ulaşıldı.");
   
   
    }
   if(h_yeni >= 200 && yedeks){ // Roketin Atılmasından önce 1600 metre yapılacak.
     tepe_noktasi2 = true; 
  ...
  if(tepe_noktasi2 && h_yeni <= 110 && (h_yeni - h_eski < -1) && (h_yeni - h_eski > -300) && (h_yeni>0) && (h_eski>0)){
    ...
    while(1){
      ...
      if(k == 2){
       ...
       delay(4500); // Simülasyonu görebilmek için asıl kodda yok.
  ...
  LoRa.print("Yukseklik:(Yedek) ");
  LoRa.println(h_yeni);
  LoRa.print("Enlem: ");
  LoRa.println(a,6);
  LoRa.print("Boylam: ");
  LoRa.println(b,6);
  LoRa.println("Yedek Uçus Bilgisayarı Gps Verisi");
